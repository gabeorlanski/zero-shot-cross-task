#!/bin/bash
# Run the experiments
python experiment.py +run_name="CT_TNF" task=anli +split=dev_r1 \
  num_proc=4 \
...
python experiment.py +run_name="CT_ASN" task=anli +split=dev_r1 \
  num_proc=4 \
  prompt_experiment_mode=cross_task \
  prompt_path=prompts/general_fixed_choice.yaml \
  model_name="bigscience/T0_3B" \
...
python experiment.py +run_name="CT_ASN" task=cb +split=validation \
  num_proc=4 \
  prompt_experiment_mode=cross_task \
  prompt_path=prompts/general_fixed_choice.yaml \
  model_name="bigscience/T0_3B" \
...
python experiment.py +run_name="CT_INC" task=cb +split=validation \
  num_proc=4 \
  prompt_experiment_mode=cross_task \
  prompt_path=prompts/general_fixed_choice.yaml \
  model_name="bigscience/T0_3B" \
